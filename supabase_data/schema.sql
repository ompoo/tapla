...
CREATE TABLE event_dates (
  ...
  date_label VARCHAR NOT NULL, -- ラベル
...
CREATE TABLE event_times (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE,
  time_label VARCHAR NOT NULL, -- ラベル
...
CREATE TABLE votes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  voteuser_id UUID NOT NULL REFERENCES voteuser(id) ON DELETE CASCADE,
  event_id UUID NOT NULL REFERENCES events(id) ON DELETE CASCADE, -- イベントID（直接参照）
  ...
  UNIQUE(voteuser_id, event_date_id, event_time_id) -- 同じvoteuserが同じスロットに重複投票を防ぐ
);
...
-- 新規認証ユーザーの最小限データ挿入関数（IDとnameを作成）
...
-- 認証ユーザー作成時のトリガー（IDのみでusersテーブルにレコード作成）
